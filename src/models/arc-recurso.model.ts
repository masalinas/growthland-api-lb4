...
@model({
  settings: {idInjection: false, mssql: {schema: 'PUBLISHER', table: 'ARC_RECURSO'}}
})
export class ArcRecurso extends Entity {
  @property({
    type: 'number',
    required: true,
    precision: 19,
    scale: 0,
    id: 1,
    mssql: {columnName: 'ID_RECURSO', dataType: 'bigint', dataLength: null, dataPrecision: 19, dataScale: 0, nullable: 'NO'},
  })
  idRecurso: number;
  ...
  @property({
    type: 'string',
    length: 100,
    mssql: {columnName: 'COD_RECURSO_EXT', dataType: 'varchar', dataLength: 100, dataPrecision: null, dataScale: null, nullable: 'YES'},
  })
  codRecursoExt?: string;
  ...
  constructor(data?: Partial<ArcRecurso>) {
    super(data);
  }
}

export interface ArcRecursoRelations {
  // describe navigational properties here
}

export type ArcRecursoWithRelations = ArcRecurso & ArcRecursoRelations;
